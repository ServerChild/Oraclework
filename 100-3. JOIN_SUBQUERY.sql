-- [ JOIN + SUBQUERY 문제 ] --
-- 1. 70년대생(1970~1979) 중 여자이면서 전씨인 사원의 이름과, 주민번호, 부서명, 직급명 조회
...
-- 8. 하정연 사원과 같은 부서에서 일하는 사원의 이름, 부서코드 조회
SELECT EMP_NAME, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE 
                                                             WHERE EMP_NAME = '하정연');
...
-- 11. 보너스 포함한 연봉이 높은 5명의 사번, 이름, 부서명, 직급명, 입사일, 순위 조회
SELECT T.* FROM (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, HIRE_DATE, RANK() OVER(ORDER BY ((SALARY * NVL(1 + BONUS, 1)) * 12) DESC) "순위"
                  FROM EMPLOYEE
                  JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
                  JOIN JOB USING (JOB_CODE)) T
    WHERE 순위 <= 5;


-- 12. 부서별 급여합계가 전체 급여 총 합의 20%보다 많은 부서의 부서명, 부서별 급여합계 조회
...
--   12-2. 인라인 뷰 사용
SELECT * FROM (SELECT DEPT_TITLE, SUM(SALARY) "부서급여합" FROM EMPLOYEE
               JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
               GROUP BY DEPT_TITLE)
    WHERE 부서급여합 > (SELECT SUM(SALARY) * 0.2 FROM EMPLOYEE);

--   12-3. WITH 사용
WITH SUDE AS (SELECT DEPT_TITLE, SUM(SALARY) "부서급여합" FROM EMPLOYEE
               JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
               GROUP BY DEPT_TITLE)

SELECT * FROM SUDE WHERE 부서급여합 > (SELECT SUM(SALARY) * 0.2 FROM EMPLOYEE);
...
-- 풀이 1
WITH SAL_SUM AS (SELECT SUM(SALARY) FROM EMPLOYEE),
     SAL_AVG AS (SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE)

SELECT * FROM SAL_SUM, SAL_AVG;
