...
/*
    - SUBSTR : 문자열에서 특정 문자열을 추출하여 반환(CHARACTER)
        -> SUBSTR(컬럼 | '문자열', POSITION, [LENGTH])
            ...
            + LENGTH : 추출할 문자 개수(생략 시 맨마지막까지 추출)
*/
...
SELECT EMP_ID, EMP_NAME, SUBSTR(EMP_NO, 8, 1) 성별 FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, RPAD(SUBSTR(EMP_NO,1,8),14,'*') FROM EMPLOYEE;

SELECT EMP_ID, EMP_NAME, SUBSTR(EMP_NO,1,8) || '******' FROM EMPLOYEE;
...
SELECT TRIM('     A I   E     ') || 'AAA' FROM DUAL;   
...
SELECT EMP_NAME, HIRE_DATE, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) 근무개월수 FROM EMPLOYEE;

SELECT EMP_NAME, HIRE_DATE, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) || '개월차' 근무개월수 FROM EMPLOYEE;

SELECT EMP_NAME, HIRE_DATE, CONCAT(CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)), '개월차') 근무개월수 FROM EMPLOYEE;
...
-----------------------------------------------------------------------------
/*
    - DECODE(비교하고자하는 대상(컬럼|산술연산|함수식), 비교값1, 결과값1, 비교값2, 결과값2, ...) -> SWITCH문이랑 비슷
...
--===========================================================================--
/*
    - CASE WHEN THEN ... END문 -> IF문이랑 비슷
      
    - 형식 : CASE WHEN 조건식1 THEN 결과값1
                  WHEN 조건식2 THEN 결과값2
                  ...
                  ELSE 결과값N
             END
